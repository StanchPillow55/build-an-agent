...
class Constraint(BaseModel):
    ...
    priority: int = Field(
        default=1, ge=1, le=5, description="Priority level (1=lowest, 5=highest)"
    )
...
class LessonPlanInput(BaseModel):
    ...
    subject_topic: str = Field(
        description="Subject or specific topic (e.g., 'Environmental Science')"
    )
    audience_baseline: str = Field(
        description="Audience knowledge baseline (e.g., 'no prior knowledge of ecosystems')"
    )
    duration_minutes: Optional[int] = Field(
        default=45, description="Lesson duration in minutes"
    )
...
class ContentSection(BaseModel):
    ...
    content_type: str = Field(
        description="Type of content (introduction, explanation, activity, etc.)"
    )
...
class Assessment(BaseModel):
    ...
    method: str = Field(
        description="Assessment method (quiz, discussion, project, etc.)"
    )
...
class LessonPlanOutput(BaseModel):
    ...
    content_breakdown: List[ContentSection] = Field(
    ...
    vocabulary: List[Dict[str, str]] = Field(
        description="Key vocabulary terms with definitions"
    )

    # Constraint compliance
    constraints_applied: List[str] = Field(
        description="List of constraints that were enforced"
    )
    age_appropriateness_notes: str = Field(
        description="Notes on age-appropriate adaptations made"
    )

    # Metadata
    created_timestamp: str = Field(description="When the lesson plan was created")
    compliance_verified: bool = Field(
        description="Whether constraint compliance was verified"
    )
