...
def collect_basic_inputs() -> dict:
    ...
    # Subject
    subject = Prompt.ask(
        "What subject or topic will you cover?",
...
def collect_output_preferences() -> dict:
    ...
    if Confirm.ask("Generate PowerPoint presentation?", default=True):
        pptx_path = Prompt.ask(
            ...
            default="lesson_presentation.pptx"
        )
    ...
    if Confirm.ask("Include OER Commons resources?", default=True):
        oer_count = typer.prompt(
            "How many OER resources to fetch?",
            type=int,
            default=3
        )
...
def wizard(
    ...
    baseline: Optional[str] = typer.Option(None, "--baseline", help="Audience knowledge baseline"),
    constraints: Optional[str] = typer.Option(None, "--constraints", help="Comma-separated constraints"),
    ...
    pptx: Optional[str] = typer.Option(None, "--pptx", help="Generate PowerPoint at path"),
    notes: bool = typer.Option(False, "--notes", help="Generate speaker notes"),
    oer: Optional[int] = typer.Option(None, "--oer", help="Number of OER resources to fetch"),
    zip_path: Optional[str] = typer.Option(None, "--zip", help="Package outputs into ZIP"),
    copilot: bool = typer.Option(False, "--copilot", help="Export to Microsoft Copilot"),
    json_only: bool = typer.Option(False, "--json-only", help="Output only raw JSON"),
    quiet: bool = typer.Option(False, "--quiet", "-q", help="Suppress progress messages")
):
    """
    ...
    try:
        if not non_interactive and not any([grade, subject]):
            ...
            if not Confirm.ask("\n[bold yellow]Proceed with curriculum generation?[/bold yellow]", default=True):
            ...
            if not grade or not subject:
                console.print("[red]Error: --grade and --subject are required in non-interactive mode[/red]")
                raise typer.Exit(1)
            ...
            params = {
                ...
                "constraints": parse_constraints(constraints or "age-appropriate,privacy-protecting"),
                "duration": duration,
                "model": model,
            }
        ...
        with Progress(
            ...
            task = progress.add_task(f"Generating curriculum plan using {params['model']}...", total=None)
        ...
        if outputs.get("pptx"):
            with Progress(
                ...
                task = progress.add_task("Generating PowerPoint presentation...", total=None)
                create_deck(plan, outputs["pptx"])
                generated_files["pptx_path"] = outputs["pptx"]
                progress.update(task, completed=100)
            
            if not quiet:
                console.print(f"[bold green]✅ Deck saved to {outputs['pptx']}[/bold green]")
        ...
        if outputs.get("notes"):
            ...
            if not quiet:
                console.print(f"[bold green]✅ Speaker notes saved to {md_path}[/bold green]")
        ...
        if outputs.get("zip"):
            with Progress(
                ...
                task = progress.add_task("Packaging outputs into ZIP file...", total=None)
                ...
                try:
                    ...
                    if not quiet:
                        console.print(f"[bold green]✅ Package saved to {package_path}[/bold green]")
        ...
        if outputs.get("copilot"):
            with Progress(
                ...
                task = progress.add_task("Exporting to Microsoft 365 OneDrive with Copilot...", total=None)
                share_url = export_to_copilot(plan)
                progress.update(task, completed=100)
                
                if not quiet:
                    console.print(f"[bold green]✅ Presentation exported to OneDrive[/bold green]")
